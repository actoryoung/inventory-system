<template>
  <div class="inbound-container">
    ...
    <!-- 搜索栏 -->
    <SearchBar @search="loadInboundList" @reset="handleReset">
      <el-select
...
function getStatusText(status: number | undefined) {
  ...
  return InboundStatusMap[status as InboundStatus] || '未知'
}

// 获取状态类型
function getStatusType(status: number | undefined) {
  if (status === undefined) return 'info'
  return InboundStatusTypeMap[status as InboundStatus] || 'info'
}

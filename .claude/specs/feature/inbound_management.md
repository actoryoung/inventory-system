...
- **规则**：
  ...
  - **并发安全（P1-2）**：取号使用 `SELECT ... FOR UPDATE` 行锁在事务内串行化，
    保证高并发下不会生成重复单号（10 线程并发创建 → 10 个单号全部唯一）
...
**限制：** 只有"待审核"状态的入库单可以删除（物理删除）
- 删除 = 物理删除（`DELETE FROM t_inbound`），不先置为作废
- "已审核" / "已作废" 状态的入库单不可删除，返回错误信息
- 删除成功后不影响库存（仅"待审核"可删，此时未产生库存变化）

**验收标准：**
- 删除"待审核"入库单 → 成功，记录被物理删除
- 删除"已审核"入库单 → 抛业务异常"只有待审核状态的入库单可以删除"
- 删除"已作废"入库单 → 抛业务异常"只有待审核状态的入库单可以删除"
...
- [x] 能够创建入库单，单号格式正确且唯一
- [x] 审核入库单后，对应商品库存自动增加
- [x] 只有待审核状态的入库单可以修改和删除（删除 = 物理删除）
- [x] 查询列表支持按商品、状态、日期筛选
- [x] 分页功能正常
- [x] 所有业务异常都有明确的错误提示
- [x] 创建入库单/出库单时商品下拉框正常加载（前端调用 `productApi.getList`）
- [x] 单号生成规则：`IN` + `yyyyMMdd` + 4位序号，全局唯一

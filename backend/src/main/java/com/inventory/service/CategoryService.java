...
public interface CategoryService extends IService<Category> {
    ...
    default boolean createCategory(Category category) {
        if (category == null) {
    ...
    default boolean updateCategory(Category category) {
    ...
    default Category getCategoryById(Long id) {
        ...
        category.setCreatedAt(vo.getCreatedAt());
        category.setUpdatedAt(vo.getUpdatedAt());
        return category;
    }

    default List<Category> getCategoryList(String name, Integer status, Integer level) {
        List<CategoryVO> source;
        if (name != null && !name.trim().isEmpty()) {
            source = searchByName(name);
        } else if (level != null) {
            source = getByLevel(level);
        } else {
            source = getList();
        }
        java.util.stream.Stream<CategoryVO> stream = source.stream();
        if (status != null) {
            stream = stream.filter(vo -> status.equals(vo.getStatus()));
        }
        return stream.map(vo -> {
            Category category = new Category();
            category.setId(vo.getId());
            category.setName(vo.getName());
            category.setParentId(vo.getParentId());
    ...
    default boolean toggleStatus(Long id) {
        Category category = getById((java.io.Serializable) id);
        if (category == null) {
            throw new RuntimeException("分类不存在");
        }
        int targetStatus = category.getStatus() != null && category.getStatus() == 1 ? 0 : 1;
